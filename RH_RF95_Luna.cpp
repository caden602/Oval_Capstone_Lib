...
RH_RF95::RH_RF95(uint8_t slaveSelectPin, uint8_t interruptPin, bool lunaSat, RHGenericSPI& spi)
    :
...
bool RH_RF95::init(bool luna)
{
    ...
    spiWrite(RH_RF95_REG_11_IRQ_FLAGS_MASK , 0x00);
...
void RH_RF95::handleInterruptBase()
{
...
//	Serial.println("R");
	...
	if (_rxBufValid)
	    //setModeIdle(); // Got one 
        setModeRx();
        // stay in RX mode until we receive stop signifier.
    }
...
void RH_RF95::handleInterruptLuna()
{
    RH_MUTEX_LOCK(lock); // Multithreading support
...
//    Serial.print("IRQ FLAGS:");
    ...
    spiWrite(RH_RF95_REG_12_IRQ_FLAGS, 0xff); // Clear all IRQ flags
    //spiWrite(RH_RF95_REG_11_IRQ_FLAGS_MASK , 0x00);
    //spiWrite(RH_RF95_REG_11_IRQ_FLAGS_MASK , 0xff); // Clear all IRQ flags
...
//	Serial.println("R");
	...
	if (_rxBufValid)
        uint8_t buf;
        uint8_t buf_global[RH_RF95_MAX_MESSAGE_LEN];
        uint8_t len_global = sizeof(buf_global);

        // if valid: read it
        if(recv(buf_global, &len_global)){
            // RH_RF95::printBuffer("Received: ", buf_global, len_global);
            // Serial.print("Got: ");
            // Serial.println((char*)buf_global);

            // Check if its our LunaSat
            char* RX = (char*)buf_global;  // get the data received
            char *ID = "LunaSat # ";

            itoa(LUNA_SAT_ID, ID+9, 3);
            
            if(!strcmp(RX,ID)){
                // maybe send one TX back that says data is coming...

                // then we schedule the event and send the rest of the data
                // in the main loop once we finish sampling.

                // Schedule an event
                add_scheduled_event(EVENT_DATA_REQUEST);
            }            
        }
        else{
...
//	Serial.println("?");
    ...
    //spiWrite(RH_RF95_REG_11_IRQ_FLAGS_MASK , 0x00); // Re-enable all IRQ flags
...
void RH_INTERRUPT_ATTR RH_RF95::isr0()
{
    if (_deviceForInterrupt[0]){
        if(_deviceForInterrupt[0]->_lunaSat){
            _deviceForInterrupt[0]->handleInterruptLuna();
        }
        else{
            _deviceForInterrupt[0]->handleInterruptBase();
        }
        
    }
    //Serial.println("INTERRUPT 0 Recieved");
}
