...
public:
    ...
    RH_RF95(uint8_t slaveSelectPin = SS, uint8_t interruptPin = 2, bool lunaSat = true, RHGenericSPI& spi = hardware_spi);
    ...
    virtual bool    init(bool lunasat);
...
protected:
    ...
    void           handleInterruptLuna();

    void           handleInterruptBase();
    ...
    // False if we are using the base station
    // True if we want to setup interrupts for LunaSat
    bool                _lunaSat;
